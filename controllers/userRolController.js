...
const createUserRol = async (req, res) => {
  ...
  try {
    const result = await UserRolService.createUserRolService({ id_rol, id_user });
...
const getUserRolById = async (req, res) => {
    const { id } = req.params;
    try {
      const result = await UserRolService.getUserRolByIdService({ id });
        if (!result) {
  ...
  const getUserRolList = async (req, res) => {
    try {
      const result = await UserRolService.getUserRolListService();
      res.status(200).json(result);
    } catch (err) {
  ...
  const deleteUserRol = async (req, res) => {
    const { id } = req.params;
    try {
      const result = await UserRolService.deleteUserRolService(id);
      res.status(200).json(result);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  const getUserRoleByUserId = async (req, res) => {
    const { id } = req.params;
    try {
      const result = await UserRolService.getUserRolByUserIdService(id);
        if (!result) {
